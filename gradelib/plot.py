...
def grade_distribution(
    ...
    hist = (
        ...
        .encode(
            x=altair.X(
                ...
                scale=altair.Scale(domain=[.6, 1.05]),
                axis=altair.Axis(title=None, values=list(gb.scale.values()) + [1])
            ),
            y=altair.Y(
                'count()',
                axis=altair.Axis(title='Count')
            )
        )
    )
    ...
    scatter = (
        altair.Chart(source)
        .mark_circle(color='red', opacity=.5)
        .encode(
            x='Score',
            y='y',
    ...
    cutoffs_source = pd.Series(gb.scale).to_frame().reset_index().rename(
        columns={0: 'thresh', 'index': 'letter'}
    )
    cutoffs_source['y'] = -2
    cutoffs_source['x'] = cutoffs_source['thresh'] + .005
    ...
    return (hist + scatter + cutoffs + letters).properties(
        width=800,
        height=400
    ).interactive()
